<div class="wrap lrob-carte-admin">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h1 style="margin: 0;"><?php esc_html_e('Products', 'lrob-la-carte'); ?></h1>
        <button class="button button-primary" id="lrob-add-product">
            <?php esc_html_e('Add Product', 'lrob-la-carte'); ?>
        </button>
    </div>

    <?php if (empty($categories)) : ?>
        <div class="notice notice-warning">
            <p><?php esc_html_e('No categories found. Please create categories first.', 'lrob-la-carte'); ?></p>
        </div>
    <?php else : ?>
        ...
        $root_categories = $categories_by_parent[0] ?? [];
        ?>
        ...
        <?php if (!empty($root_categories)) : ?>
            ...
            <div class="lrob-category-tabs" data-admin-filter-wrapper>
                ...
                ?>
                    <button class="lrob-tab lrob-admin-root-tab" data-root-category="<?php echo esc_attr($root_cat->id); ?>">
                        <span class="lrob-cat-icon">
                            ...
                            if ($root_cat->icon_type === 'emoji') {
                                ...
                                echo wp_get_attachment_image(intval($root_cat->icon_value), [24, 24]);
                            }
                            ?>
                        </span>
            ...
            <!-- Level 1 subcategory filters -->
            <?php foreach ($root_categories as $root_cat) :
                if (empty($categories_by_parent[$root_cat->id])) continue;

                $has_products = false;
                foreach ($categories_by_parent[$root_cat->id] as $child_cat) {
                    if (LRob_Carte_Database::get_products_recursive($child_cat->id)) {
                        $has_products = true;
                        break;
                    }
                }
            ...
            ?>
                <div class="lrob-subcategory-filters lrob-level-1-filters"
                     data-parent-id="<?php echo esc_attr($root_cat->id); ?>"
                     data-filter-level="1"
                        ...
                        <button class="lrob-subcategory-badge"
                            ...
                            <span class="lrob-subcategory-badge-icon">
                                ...
                                if ($child_cat->icon_type === 'emoji') {
                                    ...
                                    echo wp_get_attachment_image(intval($child_cat->icon_value), [16, 16]);
                                }
                                ?>
                            </span>
            ...
            <?php foreach ($root_categories as $root_cat) :
                ...
                foreach ($categories_by_parent[$root_cat->id] as $child_cat) :
                    ...
                    foreach ($categories_by_parent[$child_cat->id] as $grandchild_cat) {
                        if (LRob_Carte_Database::get_products_recursive($grandchild_cat->id)) {
                            $has_products = true;
                            break;
                        }
                    }
                        ...
                        <button class="lrob-subcategory-badge"
                            ...
                            <span class="lrob-subcategory-badge-icon">
                                ...
                                if ($grandchild_cat->icon_type === 'emoji') {
                                    ...
                                    echo wp_get_attachment_image(intval($grandchild_cat->icon_value), [16, 16]);
                                }
                                ?>
                            </span>
        ...
        <?php if (!empty($all_products)) : ?>
            <h2 style="margin-top: 30px; margin-bottom: 15px; font-size: 18px;">
                <?php esc_html_e('Products', 'lrob-la-carte'); ?>
            </h2>
            ...
            <div id="lrob-products-grid">
                <?php foreach ($all_products as $product) :
                    ...
                    $ancestor_ids = [$product->category_id];
                ...
                ?>
                    <div class="lrob-product-card <?php echo esc_attr($availability_class); ?>"
                         data-id="<?php echo esc_attr($product->id); ?>"
                         data-category-id="<?php echo esc_attr($product->category_id); ?>"
                         data-category-ancestors="<?php echo esc_attr(implode(',', $ancestor_ids)); ?>">
                                ...
                                <div class="lrob-product-card-main">
                                    ...
                                    <div class="lrob-product-card-badges">
                                        ...
                                        <?php if ($product->availability !== 'available') : ?>
                                            <span class="lrob-badge lrob-badge-unavailable"><?php esc_html_e('Out of Stock', 'lrob-la-carte'); ?></span>
                                        <?php endif; ?>

                                        <?php if ($product->badges) :
                                            $badges = explode(',', $product->badges);
                                            foreach ($badges as $badge) :
                                                $badge_label = LRob_Carte_Settings::get_badges()[$badge] ?? $badge;
                                        ?>
                                    ...
                                    <div class="lrob-product-card-image-desc">
                                        <?php if ($product->image_id) : ?>
                                            <div class="lrob-product-card-image">
                                                <?php echo wp_get_attachment_image(intval($product->image_id), 'thumbnail'); ?>
                                            </div>
                                        <?php endif; ?>
                                    ...
                                    <div class="lrob-product-card-prices">
                                        <?php foreach ($prices as $price) : ?>
                                            <span class="lrob-price <?php echo $price->happy_hour ? 'lrob-price-happy' : ''; ?>">
                                                ...
                                                <?php endif; ?>
                                                <strong><?php echo esc_html(number_format($price->price, 2, ',', ' ')); ?> €</strong>
                                                <?php if (!empty($price->happy_hour)) : ?>
                        ...
                        <div class="lrob-product-card-actions">
                            <button class="button button-small lrob-edit-product" data-id="<?php echo esc_attr($product->id); ?>">✏️</button>
                            <button class="button button-small lrob-delete-product" data-id="<?php echo esc_attr($product->id); ?>">🗑️</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php else : ?>
            <p class="lrob-empty"><?php esc_html_e('No products found.', 'lrob-la-carte'); ?></p>
        <?php endif; ?>
    <?php endif; ?>
</div>
...
<div id="lrob-product-modal" class="lrob-modal" style="display:none;">
    <div class="lrob-modal-content">
        ...
        <h2 id="lrob-modal-title"><?php esc_html_e('Add Product', 'lrob-la-carte'); ?></h2>

        <form id="lrob-product-form">
            <input type="hidden" id="product-id" value="">
            <input type="hidden" id="product-category" value="<?php echo esc_attr($current_cat !== 'all' ? $current_cat : ''); ?>">
            ...
            <table class="form-table">
                <tr>
                    <th><label for="product-category-select"><?php esc_html_e('Category', 'lrob-la-carte'); ?> *</label></th>
                    <td>
                        <select id="product-category-select" class="regular-text" required>
                            <option value=""><?php esc_html_e('Choose a category', 'lrob-la-carte'); ?></option>
                            <?php
                            $categories_tree = LRob_Carte_Database::get_categories_tree(true);
                            $render_category_options = function ($categories, $selected_id, $level = 0) use (&$render_category_options) {
                                foreach ($categories as $cat) {
                                    $indent = str_repeat('└─ ', $level);
                                    ?>
                                    <option value="<?php echo esc_attr($cat->id); ?>" <?php selected($selected_id, $cat->id); ?>>
                                        <?php echo esc_html($indent . $cat->name); ?>
                                    </option>
                                    <?php
                ...
                <tr>
                    <th><label for="product-name"><?php esc_html_e('Product Name', 'lrob-la-carte'); ?> *</label></th>
                    <td><input type="text" id="product-name" class="regular-text" required></td>
                </tr>
                <tr>
                    <th><label for="product-description"><?php esc_html_e('Description', 'lrob-la-carte'); ?></label></th>
                    <td><textarea id="product-description" class="large-text" rows="3"></textarea></td>
                </tr>
                <tr>
                    <th><label><?php esc_html_e('Image', 'lrob-la-carte'); ?></label></th>
                    <td>
                        <div id="product-image-preview" style="margin-bottom: 10px;"></div>
                        <input type="hidden" id="product-image-id" value="0">
                        <button type="button" class="button" id="product-upload-image"><?php esc_html_e('Choose Image', 'lrob-la-carte'); ?></button>
                        <button type="button" class="button" id="product-remove-image" style="display:none;"><?php esc_html_e('Remove', 'lrob-la-carte'); ?></button>
                    </td>
                </tr>
                <tr>
                    <th><label><?php esc_html_e('Price', 'lrob-la-carte'); ?></label></th>
                    <td>
                        <p class="description"><?php esc_html_e('Label (optional): specify a variant (e.g. "Glass (12cl)", "Pint", "Bottle"). Previously used labels will be suggested automatically.', 'lrob-la-carte'); ?></p>
                        <div id="product-prices-wrapper">
                            <div class="lrob-price-row">
                                <input type="text" class="price-label" placeholder="<?php esc_attr_e('E.g. Glass (12cl)', 'lrob-la-carte'); ?>" list="price-labels-suggestions">
                        ...
                        <button type="button" class="button" id="lrob-add-price"><?php esc_html_e('Add Price', 'lrob-la-carte'); ?></button>
                    </td>
                </tr>
                <tr>
                    <th><label><?php esc_html_e('Allergens', 'lrob-la-carte'); ?></label></th>
                    <td>
                        <div class="lrob-checkboxes">
                            <?php foreach (LRob_Carte_Settings::get_allergens() as $key => $label) : ?>
                                <label>
                                    <input type="checkbox" class="allergen-checkbox" value="<?php echo esc_attr($key); ?>">
                ...
                <tr>
                    <th><label><?php esc_html_e('Badges', 'lrob-la-carte'); ?></label></th>
                    <td>
                        <div class="lrob-checkboxes">
                            <?php foreach (LRob_Carte_Settings::get_badges() as $key => $label) : ?>
                                <label>
                                    <input type="checkbox" class="badge-checkbox" value="<?php echo esc_attr($key); ?>">
                                    <?php echo esc_html($label); ?>
                                </label>
                            <?php endforeach; ?>
                        </div>
                ...
                <tr>
                    <th><label for="product-availability"><?php esc_html_e('Availability', 'lrob-la-carte'); ?></label></th>
                    <td>
                        <select id="product-availability">
                            <option value="available"><?php esc_html_e('Available', 'lrob-la-carte'); ?></option>
                            <option value="unavailable"><?php esc_html_e('Out of Stock', 'lrob-la-carte'); ?></option>
                        </select>
                    </td>
                </tr>
            </table>
